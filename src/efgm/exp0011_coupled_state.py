"""EFGM-EXP-0011 fresh deterministic holdout generation only.

This module intentionally does NOT implement AUC calculation or scientific
classification. It exists only to generate and fingerprint the preregistered
fresh holdout before the first scientific evaluation is authorized.
"""
...
EXPERIMENT_ID = "EFGM-EXP-0011"
DATASET_VERSION = "coupled-state-fresh-holdout-v0.1"
DATASET_SEED = "EFGM-EXP-0011-FRESH-HOLDOUT-2026-08-19"
SEED_START = 110001
SEED_END = 110240
HORIZON = 24
CHECKPOINTS = (6, 12, 18)
...
def _factors_for_seed(scenario_seed: int) -> tuple[float, float, float, int]:
    ...
    if index < 0 or scenario_seed > SEED_END:
        raise ValueError("scenario seed outside frozen EXP-0011 interval")
...
def _simulate(scenario_seed: int) -> tuple[str, list[StepState], str, float, float]:
    ...
    scenario_id = f"EXP0011-S{scenario_seed}-C{capability:.2f}-U{transfer_policy:.2f}-D{entropy_band:.2f}-R{replicate}"
    reserve = 1.0
    progress = 0.0
    total_transfer = 0.0
    ...
    for step in range(1, HORIZON + 1):
        disturbance = min(0.98, max(0.0, entropy_band + _noise(scenario_seed, step) + _pulse(scenario_seed, step)))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate EXP-0011 fresh holdout manifest without scientific scoring")
    ...
    if args.format == "json":
        ...
        return
    print("# EFGM-EXP-0011 fresh holdout manifest")
    print()
